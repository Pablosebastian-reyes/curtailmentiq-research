...
Companion repository for a research article on probabilistic forecasting of renewable energy curtailment in Chile's National Electricity System (Sistema Eléctrico Nacional, SEN): *An open curtailment dataset and a diagnostic for adaptive conformal calibration under regime change* (under review; the state submitted in revision is the tag `submission-2026-09-14`, and `CHANGELOG_REVISION.md` gives the command behind every change). It also hosts the pipeline behind the associated data descriptor.
...
Revision documents, September 2026, kept in Spanish as working records: [CHANGELOG_REVISION.md](CHANGELOG_REVISION.md) (every change of the major revision, with the command that reproduces it), [REVISION_PLAN.md](REVISION_PLAN.md) (triage of the review comments and the full execution order), [HALLAZGOS_CRITICOS.md](HALLAZGOS_CRITICOS.md) (the log of critical findings, starting with H1: the adaptive gain does not survive a change of base model, which is what reframed the paper), [INFORME_VERIFICACION_INDEPENDIENTE.md](INFORME_VERIFICACION_INDEPENDIENTE.md) (the co-author's independent reproduction of Phase 0) and [BRIEFING_ESTRATEGIA.md](BRIEFING_ESTRATEGIA.md) (a dated internal briefing of 12 September, kept as a record and superseded by the submitted state).

[SEGAN-D-26-03850 (3).pdf](SEGAN-D-26-03850%20%283%29.pdf) is the manuscript as first submitted in July 2026, as returned by Editorial Manager. The revision withdraws its central claim, that the adaptive scheme narrows the intervals by about a third as a property of the method; the revised manuscript is [flagship/segan/SEGAN_paper_FINAL.tex](flagship/segan/SEGAN_paper_FINAL.tex) at the tag `submission-2026-09-14`.
...
| 5 | `python flagship/generar_figuras_paper.py` | Figures 1, 2, 3, 5 and 6 of the article in `flagship/segan/figuras/`. Figure 5, the rolling coverage, recomputes the conformal series with the same RNG order as step 3 and aborts if it does not match the official table (see [REPORTE_FIGURAS.md](flagship/segan/REPORTE_FIGURAS.md)). | 20260720 |
...
| 12 | `python flagship/revision/fase0_diagnostico.py` | `resultados/fase0/fase0_diagnostico.csv`, the diagnostic over the fifteen cells of the three-model design (Table 5, Figure 7). Needs step 9. | deterministic |
| 13 | `python flagship/revision/fase2_ablaciones.py` | `resultados/fase2/`, the marginal contribution of each component to the interval score, with day-block bootstrap intervals and runtime (Table 8, Figure 9). | 20260720, 20260901 |
| 14 | `python flagship/revision/fase3_seleccion_hiperparametros.py` | `resultados/fase3/`, the rolling-origin selection of the step and the window inside the calibration period, and the step-by-window display on the test set (Table 13). About 4 minutes. | 20260720 |
| 15 | `python flagship/revision/fase4_metricas_benchmarks.py` | `resultados/fase4/`, the one-sided interval score of every method, the four probabilistic benchmarks and the bound on alpha (Tables 9 and 10). | 20260720, 11, 20260901 |
| 16 | `python flagship/revision/fase5_dependencia_panel.py` | `resultados/fase5/`, intraclass correlation and design effect, block, group and plant-level conformal, and coverage disaggregated on four axes (Table 11). | 20260901, 20260720 |
| 17 | `python flagship/revision/fase6_cronologia.py` | `resultados/fase6/`, the chronology: 180 change-point configurations, Wasserstein distances with bootstrap intervals and permutation nulls, the seasonal decomposition and the boundary sensitivity (Tables 1 and 12). Needs step 9. | 20260901, 20260720 |
| 18 | `python flagship/revision/fase1_hiperparametros.py` | `resultados/fase1/`, Table C.14, read from the source code rather than transcribed. | deterministic |
