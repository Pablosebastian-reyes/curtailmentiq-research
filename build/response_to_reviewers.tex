...
\midrule
R1.1 & Multi-quantile GBM base model implemented; all experiments repeated on three base models; gain does not replicate; paper reframed & \S4.1, \S5.3, Tables 4--5, Fig.~7, abstract, \S8 \\
R1.2 & Algorithm in full, flowchart, complete hyperparameter table with seeds, execution order stated & \S4.5, Alg.~1, Fig.~6, App.~C \\
R1.3 & Five ablation groups with marginal contribution, bootstrap intervals and runtime & \S5.4, Table~6, Fig.~8 \\
R1.4 & Rolling-origin selection inside the calibration window; test reserved for one evaluation; sensitivity reported as display & \S4.6, \S5.7 \\
R1.5 & One-sided interval score adopted; infinite limits handled explicitly; $\alpha$ bound evaluated; four benchmarks added; all differences with day-block bootstrap & \S4.7, \S5.5, Tables 7--8 \\
R2.1 & Bibliography expanded from 10 to 43 entries, every DOI verified, no grouped citations & \S2 \\
R2.2 & \texttt{test\_ramp} renamed, footnote removed, storage mentions audited; rename verified to change labels only & \S4.7, \S5.2, Table~3, Figs.~3--4, code \\
R2.3 & Single chronology; 180 detection configurations; six alternative window boundaries evaluated & \S3.4, \S5.7, Table~11 \\
R2.4 & Explicit statement of what is new and what is taken from the literature & \S4.5 \\
R2.5 & ICC and design effect quantified; block, group and plant-level conformal tested; coverage disaggregated on four axes & \S4.3, \S5.6, Tables 9--10 \\
R2.6 & Empirical quantiles, CQR, rolling historical distributions and zero-inflated parametric benchmarks; six metrics reported & \S5.5, Table~7 \\
...
Q4 & Three new figures & Figs.~6--8 \\
Q7 & Dedicated limitations section with nine items & \S7 \\
Q8, Q9 & Restructuring and language editing throughout & whole manuscript \\
Editorial & Title shortened from 26 to 13 words, country removed, system moved to abstract & title, abstract \\
\bottomrule
\end{tabular}
...
\action{Agreed on both counts. The title went from 26 words to 13 and no longer names the country. It also now states the paper's finding rather than listing its components, which is a consequence of the reframing described in the cover letter.}
...
New: \emph{Adaptive conformal calibration under regime change: the gain measures base-model miscalibration.}

We considered two alternatives and record them here in case the Editor prefers one:

\begin{enumerate}[leftmargin=1.6em,itemsep=2pt]
\item \emph{An open curtailment dataset and a diagnostic for adaptive conformal calibration under regime change} --- gives equal weight to the two contributions.
...
\where{Title page. The description of the system, its size, its congestion and the 19{,}000~GWh curtailed over 2022 to mid-2026, has moved into the abstract, second and third sentences, so that no information is lost. The country still appears in the abstract, in Section~3 and in the keywords, where it is a statement of provenance rather than a limitation of scope.}
...
\where{Section~4.1 describes the three base models. Section~5.3 is a new subsection reporting the result, with Table~4 (the three arms in the transition window), Table~5 (all fifteen cells and the fitted relationship) and Figure~7. The abstract, the introduction (contribution paragraphs), Section~6.1 and the conclusion are rewritten around it. Section~7 states the limits of the diagnostic. The commands that regenerate it are in Appendix~D.}
...
\where{Section~4.5 gives the three items above that belong in the body. Algorithm~1 gives the full procedure in pseudocode. Figure~6 is the flowchart the reviewer asked for, with the execution order marked along the top and the two branches drawn separately so the coupling point is visible. Appendix~C, Table~12, is the complete hyperparameter specification including all four seeds. Appendix~D gives the environment and the command sequence.}
...
Tail shrinkage added on top of transport & $-0.3$ & $[-3,+2]$ & $+6.1$~s \\
Full pipeline, over the static split & $+29.3$ & $[-8,+72]$ & $+6.7$~s \\
\bottomrule
\end{tabular}
\end{center}
...
\where{Section~4.6 is a new subsection giving the protocol and the outcome. Section~5.7 reports the boundary and chronology sensitivity. The selected configuration appears as its own block in Table~7 so that it can be compared with the values of the submitted version on equal terms. The grids, the criterion and the seed are in Appendix~C.}
...
\where{Section~4.7 defines the metric and the convention for infinite bounds. Section~5.5 reports the full comparison, Table~7. Table~8 reports the effect of the $\alpha$ bound. Every difference between methods now carries a bootstrap interval that resamples whole days, in Tables 6 and 7 and in Figure~8.}
...
We did not construct a causal study of storage, and we want to say why explicitly rather than leave it implicit. The methodological contribution never depended on knowing the cause of the shift. A calibration layer that only works when the analyst can name the cause would be of little operational use, and the revision makes that explicit rather than papering over it. The evaluation window is now justified on a measurable criterion, maximum divergence from the calibration distribution, which is the property that actually matters for a conformal layer, and Section~5.7 shows the conclusions are unchanged across six alternative boundary definitions.

\where{Section~4.7 and Table~3, footnote removed and column renamed. Sections~3.4, 5.2 and 6.3. Figures~3 and 4, labels regenerated. Section~7, limitation on causal identification. Code and result files throughout the companion repository.}
...
\where{Section~3.4, the single chronology with the 180-configuration robustness check. Section~5.7 and Table~11, the boundary sensitivity. Figure~5, unchanged. Appendix~C, the penalty and the grids.}
...
\action{Section~4.5 now attributes each ingredient explicitly, and Section~4.5 states what is new. The algorithm, parameters, window length, bootstrap procedure and seeds are covered under R1.2; the ablation study under R1.3.}
...
We then report, in the same subsection, that neither the combination nor the device survives its own ablation on these data. We regard stating that plainly as more useful than the novelty claim it replaces, and it is what the reframing of the paper is built on.

\where{Section~4.5, attribution and novelty. Algorithm~1 and Figure~6, the complete procedure. Appendix~C, Table~12, every parameter and all four seeds (42 for base-model training, 20260720 for the PIT atom randomisation and the transport bootstrap, 11 for the CRPS sampling, 20260901 for the day-block bootstrap and permutation nulls). Appendix~D, environment and commands. Section~5.4, the ablations.}
...
\where{Section~4.3 is a new subsection separating the three guarantee regimes and giving the ICC and design effect. Section~5.6 reports the remedies and the disaggregated coverage, Tables~9 and 10. Section~7 states as a limitation that none of the three schemes repairs the problem. Appendix~B now reads the static guarantee at the effective rather than the nominal sample size.}
...
They changed the paper's assessment of its own method. Three of the four attain a better interval score than every scheme built on the hurdle, with day-block bootstrap intervals excluding zero. Two of them buy that with coverage, at 86.5 and 87.2 per cent, and we say so, since an operator needing the stated level cannot use them. The conformalized quantile regression is the exception and is the most informative row of the table: 89.8 per cent coverage, within half a point of nominal, at 276~MWh mean width and an interval score of 601, against 591~MWh and 1026 for the pipeline we proposed. A standard construction from 2019, on a better base predictive, dominates it.
...
\where{Section~4.7, the metric set. Section~5.5 and Table~7, the benchmarks and full comparison. Table~8, the $\alpha$ bound. Section~5.6 and Table~10, conditional coverage. Tables~6 and 7 and Figure~8 carry the bootstrap intervals.}
...
\action{Three figures added: Figure~6, the algorithm flowchart (requested in R1.2); Figure~7, the model-agnosticism result and the diagnostic relationship; Figure~8, the marginal contribution of each component with bootstrap intervals. Eight new tables: the three base models compared, the fifteen-cell diagnostic, the ablations, the benchmarks, the $\alpha$ bound, the panel-dependence schemes, the disaggregated coverage summary, the boundary sensitivity, and the complete hyperparameter specification. No existing figure was removed.}

\subsection*{Q7 --- Limitations (both reviewers: No)}

\action{Section~7 is new and dedicated, with nine items: the scope of the point-forecasting conclusion; the failure of exchangeability and the effective sample size; the fact that the diagnostic rests on three base models and one dataset; the boundaries of the multi-quantile predictive, including its quantile ceiling and its coarse occurrence probability; that differences of one to three coverage points lie within the clustered standard error; the shortness of the inner validation set; that the causes of the distributional change are not identified and cannot be with this dataset; the six descriptive statements corrected in this revision; and that the dataset records curtailment only, with no network state, dispatch, price or storage operation.}
