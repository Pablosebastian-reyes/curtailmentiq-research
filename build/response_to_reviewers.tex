...
R1.4 & Rolling-origin selection inside the calibration window; test reserved for one evaluation; sensitivity reported as display & \S4.7, \S5.8, Table~13 \\
...
R2.3 & Single chronology; 180 detection configurations; five alternative window boundaries evaluated & \S3.4, \S5.8, Table~12 \\
...
Q3 & Rolling-origin selection; one-sided interval score with an explicit convention for infinite bounds; day-block bootstrap intervals for the differences between methods & \S4.7, \S4.8, Tables~8--9 \\
Q5 & Paper reframed around the diagnostic; conclusions restricted to what the data support & abstract, \S1, \S5.3, \S6.1, \S8 \\
Q4 & Five new figures and twelve new tables & Figs.~3, 4, 7, 8 and 9; Tables~1, 4--13 and C.14 \\
Q7 & Dedicated limitations section with nine items & \S7 \\
...
\action{Agreed on both counts. The title went from 26 words to 14 and no longer names the country. It keeps the two contributions in view, the dataset and the diagnostic, and it keeps the paper identifiable as a power-systems paper: a title naming only the method was considered and set aside, because a title for this journal should say what the work is about.}
...
We state the outcome plainly, including the part that qualifies the headline figure. \textbf{At a strictly matched budget the advantage nearly disappears}: 810 trees against 800 improve the CRPS of the whole test by 2.4 per cent, not by the eighteen to thirty per cent quoted above. That figure requires roughly five thousand trees, and stated without that qualification it would attribute to the model form what is partly a matter of capacity; the manuscript now states both. What survives, and is a stronger result than the limitation it replaces, is that the hurdle cannot buy parity: given four thousand trees it gets \emph{worse}, its CRPS rising from 89.5 to 95.5~MWh, while at a comparable budget of 2{,}700 trees the multi-quantile predictive reaches 100.6 against 141.1 in the transition window. On this panel and between these two families, the multi-quantile form converts capacity into predictive accuracy and the two-part lognormal form does not. We claim nothing beyond that.

The mechanism corroborates a result already in the manuscript. When the hurdle is enlarged its estimated dispersion falls, from 1.7016 to 1.6641, and its intervals nonetheless widen by 292~MWh. The cause is the conformal quantile, which moves from 0.9242 to 0.9532; crossing the two fitted components shows that substituting only the enlarged $\mu(x)$ accounts for 93 per cent of that displacement and substituting only the smaller $\hat\sigma$ for 9.5 per cent. The degradation is of location, not of dispersion, which is the experimental counterpart of what we already report for the conditional-dispersion variant.

\textbf{The comment also let us answer the stability question on more evidence than the three base models alone provide.} Each arm of the ladder passes through the same calibration layer in the same five windows, so the diagnostic relationship can be estimated over forty cells from eight base models rather than fifteen from three. That matters because the fifteen cells span over-coverage only over $[-2.06, +5.34]$ percentage points, while the low-capacity arms reach $-8.42$ and populate the region below nominal where the three-model design had no point. The relationship strengthens: $r$ goes from $-0.755$ to $-0.873$ with a bootstrap interval of $[-0.912, -0.645]$, the slope from $-4.36$ to $-4.89$, and the intercept, which on fifteen cells was $[+0.07, +11.25]$ and barely excluded zero, becomes $[+1.60, +9.84]$. Leaving out any single cell moves $r$ within $[-0.889, -0.820]$ and leaving out any entire base model within $[-0.899, -0.792]$.
...
\item \textbf{Exact tail shrinkage function.} Now given as equation~(4) of the manuscript: $\hat q_{\mathrm{reg}}(u)=(1-\lambda(u))\hat q_{\mathrm{old}}(u)+\lambda(u)\hat q^{\mathrm{bag}}_{\mathrm{new}}(u)$ with $\lambda(u)=[1+(\mathrm{sd}_{\mathrm{boot}}(u)/\tau)^2]^{-1}$, followed by a running maximum and a clip to $[0,1]$.
...
\midrule
Online adaptation, over the static split & $-95.9$ & $[-118,-70]$ & $+0.21$~s \\
Transport alone, over the static split & $-94.2$ & $[-111,-77]$ & $+7.75$~s \\
Transport added on top of ACI & $\mathbf{+68.6}$ & $[+41,+100]$ & $+0.42$~s \\
Tail shrinkage added on top of transport & $-6.4$ & $[-10,-3]$ & $+6.16$~s \\
Full pipeline, over the static split & $-32.3$ & $[-70,+9]$ & $+6.79$~s \\
\bottomrule
\end{tabular}
\end{center}
...
Online adaptation earns its cost: $-297.9$~MWh in the transition window, $[-348,-251]$, for two tenths of a second. Score transport does not, once adaptation is present: it \emph{worsens} the interval score by 68.6~MWh over the whole test with an interval that excludes zero, and it worsens it in four of the five windows separately, significantly in three. Transport without adaptation does improve on the static split, by an amount indistinguishable from what adaptation alone achieves, at thirty-seven times the runtime; the two are substitutes and the cheaper one is at least as good. The tail shrinkage, which is the most intricate device in the construction and the one the reviewer singled out, has an effect of $-6.4$~MWh with an interval of $[-10,-3]$ while consuming 6.16 of the 6.79 seconds the full pipeline costs, ninety-one per cent of its computational budget. The interval excludes zero, so the device does something, but it is eight tenths of one per cent of an interval score of about 810 for ninety-one per cent of the runtime, and we report it as not paying for itself rather than as undetectable.

Taken together the full pipeline gains 286.5~MWh in the transition window and shows no demonstrable loss in any semester: the differences in 2025-S1 and 2025-S2, $+58.8$ and $+14.3$, have intervals containing zero, and in 2026-S1 it gains 14.7 with an interval excluding zero. Over the whole test it nets $-32.3$ with an interval containing zero: it is not distinguishable from the static split it replaces, and we do not read that as a positive result for the method.

\where{Section~5.5 is a new subsection. Table~8 gives the marginal contributions with intervals and runtimes; Figure~9 shows them window by window. Section~4.6 now states which parts of the construction are new (see R2.4) and Section~4.6 closes by noting that the combination does not survive its ablation and that the tail shrinkage survives it but does not pay for its cost, which we judged better placed there than left for the reader to discover in Section~5.}
...
\textbf{Test set.} It is used once. The $6\times5$ sensitivity analysis the reviewer asks for is reported on the test set, in Table~13 of Section~5.8, but only as a display after the selection is closed, and the manuscript says so in those words at both places where it appears, Sections~4.7 and 5.8. It shows that coverage is flat in $\gamma$ and in the window (between 89.9 and 90.1 per cent across the whole grid for the transported variant) and that the fraction of infinite intervals is what varies, from zero at $\gamma\le0.01$ to between 21.9 and 23.0 per cent at $\gamma=0.20$.

\where{Section~4.7 is a new subsection giving the protocol and the outcome. Section~5.8 reports the boundary and chronology sensitivity and, in Table~13, the $\gamma\times$window display. The selected configuration appears as its own block in Table~9 so that it can be compared with the values of the submitted version on equal terms. The grids, the criterion and the seed are in Appendix~C.}
...
\textbf{The metric.} For the one-sided interval $[0,U]$ at level $1-\alpha$ the scoring rule of Gneiting and Raftery for the quantile of that level can be written as
\[
\mathrm{IS}_\alpha(U,y)=U+\tfrac{1}{\alpha}\max(y-U,0),
\]
with multiplier $1/\alpha$ because the whole of $\alpha$ lies in the upper tail; the $2/\alpha$ of the central interval score corresponds to $\alpha/2$ in each tail. It is proper, is in MWh, penalises width and non-coverage on the same scale, and is $+\infty$ whenever $U$ is. It cannot be improved by escaping, which is precisely the property the mean finite width lacked.
...
\textbf{On verification.} Each of the forty DOIs in the bibliography was resolved, thirty-nine against Crossref and the dataset's against DataCite, where Zenodo registers it, and each record was compared with its entry for title, first author, year and journal; for the one Crossref record that lists no authors, the first author was checked against OpenAlex. The script that does it and the resulting table are in the companion repository. The other three entries are NeurIPS proceedings papers, which carry no DOI. None was added on the strength of a recollection, and we would rather submit a shorter bibliography than one we cannot verify.
...
\item Audited every remaining mention of storage, batteries and BESS in the manuscript. No part of the manuscript attributes the change causally to storage. Storage appears as one of several candidate mechanisms in Sections~1 and 6.3; as a bounded hypothesis for the trend break at the turn of 2024 to 2025 in Sections~6.3 and 7; and as literature in Sections~2.1 and 2.2. Elsewhere it appears only as information the dataset does not contain (Sections~5.1 and 7), in the observation that no storage system entered operation in June 2025 (Section~6.4), and in the data-availability statement describing the supplementary registry.
...
We did not construct a causal study of storage, and we want to say why explicitly rather than leave it implicit. The methodological contribution never depended on knowing the cause of the shift. A calibration layer that only works when the analyst can name the cause would be of little operational use, and the revision makes that explicit rather than papering over it. The evaluation window is now justified on a measurable criterion, maximum divergence from the calibration distribution, which is the property that actually matters for a conformal layer, and Section~5.8 shows the conclusions are unchanged across five alternative boundary definitions.
...
Second, on the consequences for the evaluation. We re-ran the calibration comparison on five alternative definitions of the transition window, from two months to seven, whose divergence from the calibration distribution ranges from 0.99 to 1.34. The ordering of the three methods by coverage is the same in every case, and the size of the apparent adaptive gain follows how far the static split over-covers in each window, which is the relationship established in Section~5.3.
...
We then report, in the same subsection, how each fares under ablation, and the two answers differ. The combination does not survive: with the online update in place, adding the transport worsens the interval score by 68.6~MWh over the whole test, with an interval excluding zero. The device does survive its ablation but not its price: the shrinkage improves the score by 6.4~MWh, with a bootstrap interval that also excludes zero, which is eight tenths of one per cent of the score for ninety-one per cent of the runtime. We regard stating that plainly as more useful than the novelty claim it replaces, and it is what the reframing of the paper is built on.

\where{Section~4.6, attribution and novelty. Algorithm~1 and Figure~4, the complete procedure. Appendix~C, Table~C.14, every parameter and all four seeds (42 for base-model training, 20260720 for the PIT atom randomisation and the transport bootstrap, 11 for the CRPS sampling, 20260901 for the day-block bootstrap and permutation nulls). Appendix~D, environment, seeds and a pointer to the commands in the companion repository. Section~5.5, the ablations.}
...
\textbf{Group conformal.} Mondrian calibration by technology and by capacity tercile changes little. By region it is \emph{harmful}: the worst regional deviation from nominal rises from 4.8 to 7.2 percentage points, because several regions supply too few calibration days for a stable quantile. Plant-level calibration has the same problem in stronger form, with the same 7.2-point worst regional deviation, although it and Mondrian by region have the two best marginal interval scores of the group. We report this as a negative result: partitioning the calibration set does not buy conditional validity for free when the partition is fine relative to the effective sample size.
...
They changed the paper's assessment of its own method. All four attain a better interval score than every scheme built on the hurdle, and each differs from the static split by a day-block bootstrap interval that excludes zero. Three of them buy that with coverage, at 86.5, 87.2 and 82.4 per cent, and we say so, since an operator needing the stated level cannot use them. The conformalized quantile regression is the exception and is the most informative row of the table: 89.8 per cent coverage, within half a point of nominal, at 276~MWh mean width and an interval score of 438, against 596~MWh and 813 for the pipeline we proposed. A standard construction from 2019, on a better base predictive, dominates it.
...
The abstract states that distinction in its third sentence from the end, Section~4.3 is a new subsection devoted to it, and the conclusion restates it as its penultimate paragraph.
...
\where{Abstract, third sentence from the end. Section~4.3, the three regimes. Section~5.1, the scope of the point-forecasting result. Section~7, the limitations section. Section~8, conclusion.}
...
\action{Five figures added: Figure~3, the change points of the distributional shift under the stated penalty (R2.3); Figure~4, the algorithm flowchart (requested in R1.2); Figure~7, the model-agnosticism result and the diagnostic relationship; Figure~8, the capacity ladder and the diagnostic over the forty cells; Figure~9, the marginal contribution of each component with bootstrap intervals. Twelve new tables: the seasonal decomposition of the shift, the three base models compared, the fifteen-cell diagnostic, the forty-cell diagnostic, the capacity ladder, the ablations, the benchmarks, the $\alpha$ bound, the panel-dependence schemes with their disaggregated coverage, the boundary sensitivity, the $\gamma\times$window sensitivity, and the complete hyperparameter specification. No existing figure was removed.}

\subsection*{Q7 --- Limitations (both reviewers: No)}

\action{Section~7 is new and dedicated, with nine items: the scope of the point-forecasting conclusion; the failure of exchangeability and the effective sample size; the fact that the diagnostic rests on forty cells from eight predictives that span only two model families, three hurdle variants and five budget variants of one quantile family, on a single dataset; that the separation of model form from fitted capacity rests on one panel and two model families, and that our base model is not the best hurdle available, so its levels carry a discount; the boundaries of the multi-quantile predictive, including its quantile ceiling and its coarse occurrence probability; that differences of one to three coverage points lie within the clustered standard error; the shortness of the inner validation set; that the causes of the distributional change are not identified and cannot be with this dataset; and that the dataset records curtailment only, with no network state, dispatch, price or storage operation.}

\subsection*{Q8 and Q9 --- Structure, flow and language (both reviewers: Yes)}

\action{Section~4 is now subdivided into eight subsections separating the base models, the score, the guarantees, the responses to the shift, the algorithm, the novelty statement, the selection protocol and the evaluation, which were previously interleaved. Section~5 is subdivided into eight result subsections, each answering one question. Discussion and limitations, previously mixed, are now Sections~6 and 7. The manuscript has been edited for language throughout, with attention to the passive constructions and long compound sentences of the submitted version.}
