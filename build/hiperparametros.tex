...
Nonconformity score & s = F\_x(y) (PIT) & atom at y = 0 randomised, s = (1-p)U with U $\sim$ Uniform(0,1) \\
...
Interpolation scheme & piecewise linear, twice & first s -> level on (q\_old, levels), then level -> q\_reg on (levels, q\_reg); this is the increasing rearrangement T = F\_new\textasciicircum{}\{-1\} o F\_old \\
...
Tail shrinkage function & lambda(u) = 1 / (1 + (sd\_boot(u)/tau)\textasciicircum{}2) & q\_reg(u) = (1-lambda(u)) q\_old(u) + lambda(u) q\_new\_bag(u); lambda $\sim$ 1 where the map is stable and $\sim$ 0 where it is noisy, and lambda = 0 is the identity, that is the old quantile \\
...
Primary metric & one-sided interval score IS = U + (1/alpha) max(y-U, 0) & proper for the (1-alpha) quantile, in MWh, and infinite if U is infinite; the multiplier is 1/alpha because all of alpha lies in the upper tail \\
...
Change-point detection & PELT and binary segmentation, L2 cost, pen = sigma\textasciicircum{}2 log n, min\_size = 3 & BIC penalty with k = 1; the sensitivity grid spans k in \{0.5, 1, 2, 3, 5\} and min\_size in \{2, 3, 4\} \\
\midrule
